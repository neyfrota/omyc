...
sub passwordChange{
    ...
    `/bin/echo "$p" | /opt/omyc/bin/ftpasswd --file $file_users_sftp --passwd --name $u --home /data/users/$u --shell /bin/false --uid 33 --gid 33 --stdin   >/dev/null 2>/dev/null`;
...
sub passwordDestroy{
    ...
    `/opt/omyc/bin/ftpasswd --file $file_users_sftp --name $u --delete-user --passwd >/dev/null 2>/dev/null`;
    `chmod -Rf a-rwx,u+rw $file_users_sftp >/dev/null 2>/dev/null`;
    return 1;
}
sub passwordLock{}
